...
3. **Медиа (фото, видео, кружки, голосовые и т.д.) бот сразу скачивает на диск**
   (`data/saved_media/`) — это ловит **самоуничтожающиеся сообщения**: файл
   сохраняется до того, как исчезнет с серверов Telegram
4. При удалении — `@dp.deleted_business_messages()` → бот достаёт из БД и отправляет
   владельцу текст + медиа (из локальной копии, если file_id уже мёртв)
