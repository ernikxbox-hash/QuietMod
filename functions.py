...
SAVED_MEDIA_DIR = "data/saved_media"

def _media_ext_for_msg(msg) -> str:
    """Расширение файла для локального сохранения медиа."""
    if msg.photo:          return ".jpg"
    if msg.video:          return ".mp4"
    if msg.video_note:     return ".mp4"
    ...
    if msg.document:
        name = (msg.document.file_name or "").rsplit(".", 1)
        return f".{name[1]}" if len(name) == 2 and name[1] else ".bin"
    return ".bin"

async def _save_media_to_disk(bot: Bot, owner_id: int, msg_id: int, file_id: str, ext: str) -> Optional[str]:
    """Мгновенно скачивает медиа и сохраняет на диск.

    Критично для самоуничтожающихся сообщений: после удаления файл
    исчезает с серверов Telegram, поэтому качаем сразу при получении.
    ...
    try:
        ...
        if not data:
            return None
        path = os.path.join(SAVED_MEDIA_DIR, f"{owner_id}_{msg_id}{ext}")
        with open(path, "wb") as f:
            f.write(data)
        log.info(f"💾 Медиа сохранено на диск: {path} ({len(data)} байт)")
        return path
    except Exception as e:
        log.warning(f"media save to disk: {e}")
        return None

def _find_local_media(owner_id: int, msg_id: int) -> Optional[str]:
    """Ищет ранее сохранённое медиа для (owner_id, msg_id)."""
    try:
        import glob
        matches = glob.glob(os.path.join(SAVED_MEDIA_DIR, f"{owner_id}_{msg_id}.*"))
        return matches[0] if matches else None
    except Exception:
        return None

def _remove_local_media(owner_id: int, msg_id: int):
    """Удаляет сохранённое медиа с диска (после отправки владельцу)."""
    path = _find_local_media(owner_id, msg_id)
    if path:
        try:
            os.remove(path)
        except Exception:
...
# Метаданные недавних медиа-сообщений (owner_id, msg_id) -> dict.
# Запасной источник для on_deleted: delete-событие самоуничтожающегося сообщения
# может прийти раньше, чем батч-райтер запишет строку в SQLite.
recent_media_meta: dict[tuple[int, int], dict] = {}
RECENT_MEDIA_MAX = 500

def _store_media_meta(owner_id: int, msg_id: int, meta: dict):
    recent_media_meta[(owner_id, msg_id)] = meta
    if len(recent_media_meta) > RECENT_MEDIA_MAX:
        for k in list(recent_media_meta)[: len(recent_media_meta) - RECENT_MEDIA_MAX]:
            recent_media_meta.pop(k, None)

def _get_media_meta(owner_id: int, msg_id: int) -> Optional[dict]:
    return recent_media_meta.get((owner_id, msg_id))
